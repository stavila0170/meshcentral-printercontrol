...
module.exports.printercontrol = function (parent) {
    ...
    obj.server_startup = function () {
        ...
        obj.debug("plugin:printercontrol", "Printer Control 0.4.41 started with file-based watcher launch, shared startup acknowledgement and PrintService fallback capture");
    };
        ...
        if (Array.isArray(event.jobs)) {
            for (var i = 0; i < event.jobs.length && i < 250; i++) {
                ...
                jobs.push({
                    ...
                    trackingMode: typeof job.trackingMode === "string" ? job.trackingMode.substring(0, 32) : "",
                    estimatedSecondsRemaining: typeof job.estimatedSecondsRemaining === "number" ? Math.max(0, Math.floor(job.estimatedSecondsRemaining)) : parseInt(job.estimatedSecondsRemaining, 10) || 0,
                    spoolerOwned: job.spoolerOwned !== false
                });
            }
        }
        var printers = [];
