...
function buildJobWatcherScript() {
    return [
        ...
        "      if ($jobCount -gt 0 -and $liveState -notmatch '^(Offline|Paper Jam|Paper Out|Door Open|No Toner|Error|Stopped)$') { $liveState='Printing' }",
        ...
        "        physicalStatusReported=$false",
        "        deviceState=''",
        "        trackingMode='Spooler'",
        "        estimatedSecondsRemaining=0",
        "        spoolerOwned=$true",
        "      }",
        "      $job['fingerprint']=('{0}|{1}|{2}|{3}|{4}|{5}' -f $job.documentName,$job.userName,$job.rawJobStatus,$job.totalPages,$job.pagesPrinted,$job.size)",
        "      $current[$name]=$job",
        "    }",
        "    return $current",
        "  } catch { return $null }",
        ...
        "  $seconds=12+($remaining*6)",
        "  if ([long]$job.size -gt 10485760) { $seconds+=20 }",
        "  return [int][Math]::Min(300,[Math]::Max(20,$seconds))",
        "}",
        ...
        "    if ($errorState -eq 8) { return [ordered]@{ state='Paper Jam'; reported=$true; busy=$true; idle=$false } }",
        "    if ($errorState -eq 4) { return [ordered]@{ state='Paper Out'; reported=$true; busy=$true; idle=$false } }",
        "    if ($errorState -eq 7) { return [ordered]@{ state='Door Open'; reported=$true; busy=$true; idle=$false } }",
        "    if ($errorState -eq 6) { return [ordered]@{ state='No Toner'; reported=$true; busy=$true; idle=$false } }",
        "    if ($extended -eq 9 -or $errorState -in @(10,11)) { return [ordered]@{ state='Error'; reported=$true; busy=$true; idle=$false } }",
        "    if ($printerStatus -eq 6 -or $extended -in @(6,8)) { return [ordered]@{ state='Stopped'; reported=$true; busy=$true; idle=$false } }",
        "    if ($printerStatus -eq 5 -or $extended -in @(5,14)) { return [ordered]@{ state='Warming up'; reported=$true; busy=$true; idle=$false } }",
        ...
        "      if ($device.state -match '^(Offline|Paper Jam|Paper Out|Door Open|No Toner|Error|Stopped)$') { $physical.jobStatus=[string]$device.state } else { $physical.jobStatus='Printing' }",
        "    } elseif ($device.idle -and $physical.busySeen) {",
        "      $physical.idleSamples++",
        "      $physical.trackingMode='Device'",
        "      $physical.jobStatus='Printing'",
        "      if ($physical.idleSamples -ge 4) { $complete=$true }",
        ...
        "      if ($device.idle) { $physical.deviceState='Idle (physical progress is not exposed by this driver)' }",
        ...
        "    }",
        "    $physical=[ordered]@{ key=$key; printerName=$old.printerName; id=$old.id; documentName=$old.documentName; userName=$old.userName; jobStatus='Printing'; totalPages=$old.totalPages; pagesPrinted=$old.pagesPrinted; size=$old.size; submittedTime=$old.submittedTime; statusReported=$false; deviceState='Waiting for printer status'; trackingMode='Estimated'; busySeen=$false; idleSamples=0; estimatedUntil=$estimateStart.AddSeconds($duration); expiresAt=$now.AddMinutes(9); lastPublished='' }",
