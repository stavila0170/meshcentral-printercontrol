...
function buildJobWatcherScript() {
    return [
        ...
        "$script:lastSuccessfulPrintAt=@{}",
        "$script:unresolvedActivityAt=@{}",
        "$script:lastUnresolvedSignal=(Get-Date).AddYears(-1)",
        ...
        "  if ($combined -match 'paper.?problem|check.?paper') { return 'Paper Problem' }",
        ...
        "        $physicalItem=$script:physicalJobs[$physicalKey]",
        "        if ($physicalItem.printerName -eq $name) {",
        "          $physicalCount++",
        "          if ([string]::IsNullOrWhiteSpace([string]$physicalFault) -and [string]$physicalItem.jobStatus -match '^(Offline|Paper Jam|Paper Out|Paper Problem|Door Open|No Toner|Output Bin Full|Out of Memory|Manual Feed|User Intervention|Error|Stopped|Paused)$') { $physicalFault=[string]$physicalItem.jobStatus }",
        "        }",
        "      }",
        ...
        "      $printerKey=$name.ToLowerInvariant()",
        "      $now=Get-Date",
        "      $unresolvedAt=$null",
        "      if ($script:unresolvedActivityAt.ContainsKey($printerKey)) { $unresolvedAt=[datetime]$script:unresolvedActivityAt[$printerKey] }",
        "      $successAt=$null",
        "      if ($script:lastSuccessfulPrintAt.ContainsKey($printerKey)) { $successAt=[datetime]$script:lastSuccessfulPrintAt[$printerKey] }",
        "      if ($jobCount -gt 0 -and $script:unresolvedActivityAt.ContainsKey($printerKey)) { $script:unresolvedActivityAt.Remove($printerKey); $unresolvedAt=$null }",
        "      if ($null -ne $unresolvedAt -and $null -ne $successAt -and $successAt -ge $unresolvedAt) { $script:unresolvedActivityAt.Remove($printerKey); $unresolvedAt=$null }",
        "      if ($null -ne $unresolvedAt -and ($now-$unresolvedAt).TotalMinutes -ge 5) { $script:unresolvedActivityAt.Remove($printerKey); $unresolvedAt=$null }",
        "      if (-not [string]::IsNullOrWhiteSpace([string]$physicalFault)) { $liveState=[string]$physicalFault }",
        "      elseif (-not [string]::IsNullOrWhiteSpace([string]$queueFault)) { $liveState=[string]$queueFault }",
        ...
        "      elseif ($null -ne $unresolvedAt -and ($now-$unresolvedAt).TotalSeconds -ge 20) { $liveState='Paper Problem' }",
        "      elseif ($null -ne $unresolvedAt) { $liveState='Printing' }",
        ...
        "    $printerKey=([string]$printerName).ToLowerInvariant()",
        "    $script:lastSuccessfulPrintAt[$printerKey]=$now",
        "    if ($script:unresolvedActivityAt.ContainsKey($printerKey)) { $script:unresolvedActivityAt.Remove($printerKey) }",
        ...
        "    $physical=[ordered]@{ key=$key; printerName=[string]$printerName; id=[int]$jobId; documentName=[string]$document; userName=[string]$owner; jobStatus='Printing'; totalPages=[int]$pages; pagesPrinted=0; size=[long]$size; submittedTime=$(if ($null -ne $event.TimeCreated) { $event.TimeCreated.ToUniversalTime().ToString('o') } else { $now.ToUniversalTime().ToString('o') }); statusReported=$false; deviceState='Captured from Windows PrintService event 307'; trackingMode='PrintService event fallback'; busySeen=$false; idleSamples=0; estimatedUntil=$estimatedUntil; hardUntil=$estimatedUntil.AddSeconds(8); expiresAt=$now.AddMinutes(9); attentionRaised=$false; lastPublished='' }",
        ...
        "      $physical.attentionRaised=$false",
        ...
        "    } elseif ($device.busy -and $now -lt $physical.hardUntil) {",
        "      $physical.busySeen=$true",
        "      $physical.idleSamples=0",
        "      $physical.trackingMode='Device'",
        "      $physical.jobStatus='Printing'",
        "    } elseif ($now -ge $physical.hardUntil) {",
        "      $physical.idleSamples=0",
        "      $physical.attentionRaised=$true",
        "      $physical.jobStatus='Paper Problem'",
        "      $physical.deviceState='No physical progress detected; check for Paper Out or Paper Jam'",
        "      $physical.trackingMode='Paper attention heuristic'",
        "    } else {",
        "      $physical.idleSamples=0",
        "      $physical.jobStatus='Printing'",
        ...
        "  $now=Get-Date",
        "  if (($now-$script:lastUnresolvedSignal).TotalSeconds -lt 2) { return 0 }",
        "  $script:lastUnresolvedSignal=$now",
        "  $printerName=''",
        "  if (@($script:knownPrinterNames).Count -eq 1) { $printerName=[string]$script:knownPrinterNames[0] }",
        "  if ([string]::IsNullOrWhiteSpace([string]$printerName)) {",
        "    try {",
        "      $defaultPrinter=@(Get-CimInstance Win32_Printer -ErrorAction SilentlyContinue | Where-Object { $_.Default -eq $true } | Select-Object -First 1)",
        "      if ($defaultPrinter.Count -gt 0) { $printerName=[string]$defaultPrinter[0].Name }",
        "    } catch { }",
        "  }",
        "  if ([string]::IsNullOrWhiteSpace([string]$printerName)) { return 0 }",
        "  $printerKey=$printerName.ToLowerInvariant()",
        "  $script:unresolvedActivityAt[$printerKey]=$now",
        "  return 1",
        "}",
        "try {",
