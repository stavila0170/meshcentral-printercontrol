...
function buildJobWatcherScript() {
    return [
        ...
        "    try {",
        "      $jobText=(@(Get-PrintJob -PrinterName $printerName -ErrorAction SilentlyContinue | ForEach-Object { [string]$_.JobStatus } | Where-Object { $_ } | Sort-Object -Unique) -join ', ')",
        "    } catch { $jobText='' }",
        "  } catch { }",
        "  $result=[ordered]@{ printerStatus=$printerText; jobStatus=$jobText; timestamp=$now }",
        "  $script:printerSignalCache[$key]=$result",
        "  return $result",
        "}",
        "function Resolve-LivePrinterState($printerStatus,$extended,$errorState,$workOffline,$printerText,$jobText) {",
        "  $combined=(([string]$printerText)+'|'+([string]$jobText)).ToLowerInvariant()",
        "  if ([bool]$workOffline -or $printerStatus -eq 7 -or $extended -in @(7,11) -or $errorState -eq 9 -or $combined -match 'offline|not.?available|server.?unknown') { return 'Offline' }",
        "  if ($errorState -eq 8 -or $combined -match 'paper.?jam') { return 'Paper Jam' }",
        "  if ($errorState -eq 4 -or $combined -match 'paper.?out|no.?paper') { return 'Paper Out' }",
        ...
        "  if ($combined -match 'user.?intervention|attention|blocked') { return 'User Intervention' }",
        "  if ($extended -eq 9 -or $errorState -in @(10,11) -or $combined -match '(^|[^a-z])error([^a-z]|$)|paper.?problem|page.?punt') { return 'Error' }",
        "  if ($printerStatus -eq 6 -or $extended -eq 6 -or $combined -match 'stopped') { return 'Stopped' }",
        "  if ($extended -eq 8 -or $combined -match 'paused') { return 'Paused' }",
        ...
        "  if ($printerStatus -eq 4 -or $extended -in @(4,10,12,13,17) -or $combined -match 'printing|busy|processing|spooling|i/?o.?active|waiting') { return 'Printing' }",
        ...
        "  return Resolve-LivePrinterState ([int]$item.PrinterStatus) ([int]$item.ExtendedPrinterStatus) ([int]$item.DetectedErrorState) ([bool]$item.WorkOffline) ([string]$signals.printerStatus) ([string]$signals.jobStatus)",
        "}",
        ...
        "      if ($jobCount -gt 0 -and $liveState -notmatch '^(Offline|Paper Jam|Paper Out|Door Open|No Toner|Output Bin Full|Out of Memory|Manual Feed|User Intervention|Error|Stopped|Paused)$') { $liveState='Printing' }",
        ...
        "      if (-not $script:firstSeenAt.ContainsKey($name)) { $script:firstSeenAt[$name]=Get-Date }",
        "      $rawStatus=[string]$item.JobStatus",
        "      if ([string]::IsNullOrWhiteSpace($rawStatus)) { $rawStatus='Queued' }",
        "      $displayStatus=if ($rawStatus -match '(?i)printed|completed|complete') { 'Printing' } else { $rawStatus }",
        ...
        "        trackingMode='Spooler'",
        "        estimatedSecondsRemaining=0",
        "        spoolerOwned=$true",
        "      }",
        "      $job['fingerprint']=('{0}|{1}|{2}|{3}|{4}|{5}|{6}|{7}' -f $job.documentName,$job.userName,$job.rawJobStatus,$job.totalPages,$job.pagesPrinted,$job.size,$job.jobStatus,$job.deviceState)",
        "      $current[$name]=$job",
        "    }",
        "    return $current",
        "  } catch { return $null }",
        ...
        "      $state=Resolve-LivePrinterState ([int]$device[0].PrinterStatus) ([int]$device[0].ExtendedPrinterStatus) ([int]$device[0].DetectedErrorState) ([bool]$device[0].WorkOffline) ([string]$signals.printerStatus) ([string]$signals.jobStatus)",
