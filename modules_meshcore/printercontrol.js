...
function buildJobWatcherScript() {
    return [
        ...
        "  $printerNames=@($script:knownPrinterNames)",
        "  if ($printerNames.Count -eq 0) {",
        "    try { $printerNames=@(Get-Printer -ErrorAction Stop | Select-Object -ExpandProperty Name) } catch { $printerNames=@() }",
        "  }",
        ...
        "  $now=Get-Date",
        "  if ($forceDirect -or (($now-$script:lastCmdletJobPoll).TotalMilliseconds -ge 200)) {",
