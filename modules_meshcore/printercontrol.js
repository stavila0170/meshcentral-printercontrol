...
function buildJobWatcherScript() {
    return [
        ...
        "$script:lastSnapshotPublish=(Get-Date).AddYears(-1)",
        "$notifier=$null",
        ...
        "}",
        "function Publish-FullQueueSnapshots($current) {",
        "  $now=Get-Date",
        "  if (($now-$script:lastSnapshotPublish).TotalMilliseconds -lt 2000) { return 0 }",
        "  $script:lastSnapshotPublish=$now",
        "  $printerNames=@{}",
        "  foreach ($job in @($current.Values)) { if (-not [string]::IsNullOrWhiteSpace([string]$job.printerName)) { $printerNames[[string]$job.printerName]=$true } }",
        "  foreach ($physicalKey in @($script:physicalJobs.Keys)) {",
        "    $physical=$script:physicalJobs[$physicalKey]",
        "    if ($null -ne $physical -and -not [string]::IsNullOrWhiteSpace([string]$physical.printerName)) { $printerNames[[string]$physical.printerName]=$true }",
        "  }",
        "  $published=0",
        "  foreach ($printerName in @($printerNames.Keys)) {",
        "    $jobs=@(Get-DisplayJobsForPrinter $printerName $current)",
        "    if ($jobs.Count -eq 0) { continue }",
        "    $payload=[ordered]@{ eventType='FullQueueSnapshotEvent'; printerName=[string]$printerName; jobId=0; document=''; owner=''; status='Snapshot'; timestamp=(Get-Date).ToUniversalTime().ToString('o'); jobs=$jobs }",
        ...
        "  if ($null -ne $initial) { [void](Publish-PrintJobChanges $initial); [void](Publish-FullQueueSnapshots $initial) }",
        ...
        "        }",
        "        if ($null -ne $current) { [void](Publish-PhysicalPrinterStates $current); [void](Publish-FullQueueSnapshots $current) }",
        "        if ($burstChanges -eq 0) { Publish-UnresolvedSpoolerActivity }",
        "      } else {",
        "        $current=Read-PrintJobSnapshot",
        "        if ($null -ne $current) { [void](Publish-PrintJobChanges $current); [void](Publish-PhysicalPrinterStates $current); [void](Publish-FullQueueSnapshots $current) }",
        "      }",
        "    } else {",
        "      $current=Read-PrintJobSnapshot",
        "      if ($null -ne $current) { [void](Publish-PrintJobChanges $current); [void](Publish-PhysicalPrinterStates $current); [void](Publish-FullQueueSnapshots $current) }",
