...
var WATCHER_MARKER = "MESH_PRINTERCONTROL_WATCHER_0425";
...
function buildJobWatcherScript() {
    return [
        ...
        "      $job=$current[$key]",
        "      $eventClass=$null",
        "      if (-not $previous.ContainsKey($key)) { $eventClass='__InstanceCreationEvent' }",
        "      elseif ($previous[$key].fingerprint -ne $job.fingerprint) { $eventClass='__InstanceModificationEvent' }",
        "      if ($null -eq $eventClass) { continue }",
        "      $jobs=@($current.Values | Where-Object { $_.printerName -eq $job.printerName } | Select-Object -First 250)",
        ...
        "      $old=$previous[$key]",
        "      $deleted=[ordered]@{ id=$old.id; documentName=$old.documentName; userName=$old.userName; jobStatus='Sent to printer'; totalPages=$old.totalPages; pagesPrinted=$old.pagesPrinted; size=$old.size; submittedTime=$old.submittedTime; recentCompleted=$true }",
        "      $remaining=@($current.Values | Where-Object { $_.printerName -eq $old.printerName } | Select-Object -First 249)",
        "      $jobs=@($deleted)+$remaining",
        "      $payload=[ordered]@{ eventType='__InstanceDeletionEvent'; printerName=$old.printerName; jobId=$old.id; document=$old.documentName; owner=$old.userName; status='Sent to printer'; timestamp=(Get-Date).ToUniversalTime().ToString('o'); jobs=$jobs }",
        "      [Console]::Out.WriteLine(($payload|ConvertTo-Json -Compress -Depth 4))",
        "      [Console]::Out.Flush()",
        "    }",
        "    $previous=$current",
