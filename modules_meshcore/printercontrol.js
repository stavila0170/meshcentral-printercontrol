...
var WATCHER_MARKER = "MESH_PRINTERCONTROL_WATCHER_0424";
...
function buildJobWatcherScript() {
    return [
        ...
        // The previous one-second WMI sampling window could completely miss a
        // fast TCP/IP job. Live monitoring is explicitly opt-in and bounded to
        // ten minutes, so use a short window while the watcher is active.
        "$query=\"SELECT * FROM __InstanceOperationEvent WITHIN 0.2 WHERE TargetInstance ISA 'Win32_PrintJob'\"",
        ...
        "    try { $evt=$watcher.WaitForNextEvent() } catch [System.Management.ManagementException] { if ($_.Exception.ErrorCode -eq [System.Management.ManagementStatus]::Timedout) { continue } throw }",
