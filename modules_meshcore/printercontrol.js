...
var WATCHER_MARKER = "MESH_PRINTERCONTROL_WATCHER_0444";
...
function buildJobWatcherScript() {
    return [
        ...
        "  [uint32]$printerState=[uint32]$item.PrinterState",
        "  if ([bool]$item.WorkOffline -or $printerStatus -eq 7 -or $extended -eq 7 -or $errorState -eq 9 -or $printerState -in @(8,13,24)) { return $false }",
        "  return $true",
        "}",
        ...
        "  return $result",
        "}",
        "function Resolve-LivePrinterState($printerStatus,$extended,$errorState,$printerState,$workOffline,$printerText,$jobText,$jobMask,$extraText,$extendedErrors) {",
        "  $combined=(([string]$printerText)+'|'+([string]$jobText)+'|'+([string]$extraText)).ToLowerInvariant()",
        "  [uint32]$mask=[uint32]$jobMask",
        "  [uint32]$stateMask=[uint32]$printerState",
        "  $extendedErrorValues=@($extendedErrors | ForEach-Object { try { [int]$_ } catch { } })",
        "  if ([bool]$workOffline -or $printerStatus -eq 7 -or $extended -in @(7,11) -or $errorState -eq 9 -or $extendedErrorValues -contains 15 -or $stateMask -in @(8,13,24) -or (($mask -band 32) -ne 0) -or $combined -match 'offline|not.?available|server.?unknown') { return 'Offline' }",
        "  if ($errorState -eq 8 -or $extendedErrorValues -contains 8 -or $stateMask -eq 4 -or $combined -match 'paper.?jam|jammed') { return 'Paper Jam' }",
        "  if ($errorState -eq 4 -or $extendedErrorValues -contains 4 -or $stateMask -eq 5 -or (($mask -band 64) -ne 0) -or $combined -match 'paper.?out|no.?paper|out.?of.?paper|tray.?empty') { return 'Paper Out' }",
        "  if ($extendedErrorValues -contains 11 -or $stateMask -eq 7 -or $combined -match 'paper.?problem') { return 'Paper Problem' }",
        "  if ($errorState -eq 7 -or $extendedErrorValues -contains 7 -or $stateMask -eq 23 -or $combined -match 'door.?open') { return 'Door Open' }",
        "  if ($errorState -eq 6 -or $extendedErrorValues -contains 6 -or $stateMask -eq 19 -or $combined -match 'no.?toner|toner.?empty') { return 'No Toner' }",
        "  if ($errorState -eq 11 -or $extendedErrorValues -contains 10 -or $stateMask -eq 12 -or $combined -match 'output.?bin.?full') { return 'Output Bin Full' }",
        "  if ($extendedErrorValues -contains 14 -or $stateMask -eq 22 -or $combined -match 'out.?of.?memory') { return 'Out of Memory' }",
        "  if ($stateMask -eq 6 -or $combined -match 'manual.?feed') { return 'Manual Feed' }",
        "  if ($stateMask -in @(20,21) -or ($extendedErrorValues -contains 12) -or ($extendedErrorValues -contains 13) -or (($mask -band 1024) -ne 0) -or (($mask -band 512) -ne 0) -or $combined -match 'user.?intervention|attention|blocked|page.?punt') { return 'User Intervention' }",
        "  if ($stateMask -eq 2 -or $extended -eq 9 -or $errorState -eq 10 -or $extendedErrorValues -contains 9 -or (($mask -band 2) -ne 0) -or $combined -match '(^|[^a-z])error([^a-z]|$)|service.?requested') { return 'Error' }",
        "  if ($printerStatus -eq 6 -or $extended -eq 6 -or $combined -match 'stopped') { return 'Stopped' }",
        "  if ($stateMask -eq 1 -or $extended -eq 8 -or (($mask -band 1) -ne 0) -or $combined -match 'paused') { return 'Paused' }",
        "  if ($errorState -eq 5 -or $extendedErrorValues -contains 5 -or $stateMask -eq 18 -or $combined -match 'toner.?low') { return 'Toner Low' }",
        "  if ($errorState -eq 3 -or $extendedErrorValues -contains 3 -or $combined -match 'paper.?low') { return 'Paper Low' }",
        "  if ($printerStatus -eq 5 -or $extended -in @(5,14) -or $stateMask -in @(16,17) -or $combined -match 'warming|initializ') { return 'Warming up' }",
        "  if ((($mask -band 24) -ne 0) -or $stateMask -in @(9,10,11,14,15) -or $printerStatus -eq 4 -or $extended -in @(4,10,12,13,17) -or $combined -match 'printing|busy|processing|spooling|i/?o.?active|waiting') { return 'Printing' }",
        "  if ($printerStatus -eq 3 -or $extended -in @(3,15) -or $stateMask -in @(0,25) -or $combined -match 'idle|ready|normal|power.?save') { return 'Idle' }",
        "  return 'Ready'",
        "}",
        ...
        "  if ($combined -match 'paper.?jam|jammed') { return 'Paper Jam' }",
        "  if ((($mask -band 64) -ne 0) -or $combined -match 'paper.?out|no.?paper|out.?of.?paper|tray.?empty') { return 'Paper Out' }",
        "  if ($combined -match 'paper.?problem') { return 'Paper Problem' }",
        ...
        "  foreach ($candidate in @('Offline','Paper Jam','Paper Out','Paper Problem','Door Open','No Toner','Output Bin Full','Out of Memory','Manual Feed','User Intervention','Error','Stopped','Paused')) {",
        ...
        "  $extraText=(([string]$item.Status)+'|'+([string]$item.ErrorDescription)+'|'+(@($item.ErrorInformation)-join ',')+'|'+(@($item.ExtendedDetectedErrorState)-join ','))",
        "  return Resolve-LivePrinterState ([int]$item.PrinterStatus) ([int]$item.ExtendedPrinterStatus) ([int]$item.DetectedErrorState) ([uint32]$item.PrinterState) ([bool]$item.WorkOffline) ([string]$signals.printerStatus) ([string]$signals.jobStatus) ([uint32]$signals.jobStatusMask) $extraText @($item.ExtendedDetectedErrorState)",
        "}",
        "function Read-ActivePrinterSnapshot($current) {",
        "  $printerQuery=New-Object System.Management.ObjectQuery('SELECT Name,DriverName,PortName,Shared,ShareName,Default,PrinterStatus,ExtendedPrinterStatus,DetectedErrorState,ExtendedDetectedErrorState,PrinterState,Status,StatusInfo,Availability,ErrorDescription,ErrorInformation,WorkOffline FROM Win32_Printer')",
        ...
        "      elseif ($jobCount -gt 0 -and $liveState -notmatch '^(Offline|Paper Jam|Paper Out|Paper Problem|Door Open|No Toner|Output Bin Full|Out of Memory|Manual Feed|User Intervention|Error|Stopped|Paused)$') { $liveState='Printing' }",
        ...
        "  return 1",
        "}",
        "function Apply-PaperStallHeuristic($key,$job,$device) {",
        "  if ($null -eq $job -or [string]::IsNullOrWhiteSpace([string]$key)) { return }",
        "  $now=Get-Date",
        "  $signature=('{0}|{1}|{2}|{3}' -f [int]$job.pagesPrinted,[uint32]$job.statusMask,[string]$job.rawJobStatus,[string]$job.statusText)",
        "  if (-not $script:jobProgress.ContainsKey($key)) {",
        "    $script:jobProgress[$key]=[ordered]@{ lastSignature=$signature; lastProgressAt=$now; createdAt=$now }",
        "  } else {",
        "    $progress=$script:jobProgress[$key]",
        "    if ([string]$progress.lastSignature -ne $signature) { $progress.lastSignature=$signature; $progress.lastProgressAt=$now }",
        "  }",
        "  if ([bool]$device.fault -or [bool]$device.paused) { return }",
        "  if ([string]$job.jobStatus -match '^(Offline|Paper Jam|Paper Out|Paper Problem|Door Open|No Toner|Output Bin Full|Out of Memory|Manual Feed|User Intervention|Error|Stopped|Paused)$') { return }",
        "  $progress=$script:jobProgress[$key]",
        "  $estimated=[int](Get-EstimatedPhysicalSeconds $job)",
        "  $threshold=[int][Math]::Max(18,[Math]::Min(75,$estimated+5))",
        "  $age=($now-[datetime]$job.firstSeenAt).TotalSeconds",
        "  $quiet=($now-[datetime]$progress.lastProgressAt).TotalSeconds",
        "  if ($age -ge $threshold -and $quiet -ge $threshold -and [string]$job.jobStatus -match '^(Printing|Spooling|Queued|Waiting|Processing|Idle|Ready|Normal)$') {",
        "    $job.jobStatus='Paper Problem'",
        "    $job.deviceState='No print progress detected; possible Paper Out or Paper Jam'",
        "    $job.physicalStatusReported=$false",
        "    $job.trackingMode=(([string]$job.trackingMode)+' + stalled paper check').Trim(' ','+')",
        "  }",
        "}",
        ...
        "    spoolerOwned=$true",
        "  }",
        "  Apply-PaperStallHeuristic $key $job $device",
        ...
        "        Apply-PaperStallHeuristic $key $existing $device",
        ...
        "  $deviceQuery=New-Object System.Management.ObjectQuery(\"SELECT PrinterStatus,ExtendedPrinterStatus,DetectedErrorState,ExtendedDetectedErrorState,PrinterState,Status,StatusInfo,Availability,ErrorDescription,ErrorInformation,WorkOffline FROM Win32_Printer WHERE Name='$escaped'\")",
        ...
        "      $extraText=(([string]$device[0].Status)+'|'+([string]$device[0].ErrorDescription)+'|'+(@($device[0].ErrorInformation)-join ',')+'|'+(@($device[0].ExtendedDetectedErrorState)-join ','))",
        "      $state=Resolve-LivePrinterState ([int]$device[0].PrinterStatus) ([int]$device[0].ExtendedPrinterStatus) ([int]$device[0].DetectedErrorState) ([uint32]$device[0].PrinterState) ([bool]$device[0].WorkOffline) ([string]$signals.printerStatus) ([string]$signals.jobStatus) ([uint32]$signals.jobStatusMask) $extraText @($device[0].ExtendedDetectedErrorState)",
        "      $fault=($state -match '^(Offline|Paper Jam|Paper Out|Paper Problem|Door Open|No Toner|Output Bin Full|Out of Memory|Manual Feed|User Intervention|Error|Stopped)$')",
        ...
        "      if ($script:jobProgress.ContainsKey($key)) { $script:jobProgress.Remove($key) }",
