...
function startJobWatcher(requestId, params) {
    ...
    try {
        ...
        watcherScript = buildJobWatcherScript();

        // MeshAgent's embedded fs.writeFileSync implementation is not Node-compatible
        // on every agent build and can throw "number required, found undefined".
        // Also, waiting for ReadToEnd() can stall because the embedded stdin stream
        // does not always deliver EOF. Send the exact ASCII character count instead:
        // PowerShell reads that many characters and starts immediately without a file
        // and without requiring stdin.end().
        var bootstrap =
            "$n=" + watcherScript.length + ";" +
            "$b=New-Object char[] $n;$o=0;" +
            "while($o -lt $n){" +
            "$r=[Console]::In.Read($b,$o,$n-$o);" +
            "if($r -le 0){throw 'Unexpected end of watcher input'};" +
            "$o+=$r};" +
            "$pcScript=-join $b;" +
            "& ([ScriptBlock]::Create($pcScript))";
        ...
        sendResult(requestId, "watchJobsStart", { success: false, error: "Unable to launch print-job watcher: " + startError });
        return;
    }
    ...
    try {
        if (!child.stdin) throw new Error("PowerShell standard input is unavailable");
        // Small writes avoid pipe-buffer limitations in older MeshAgent builds.
        for (var watcherOffset = 0; watcherOffset < watcherScript.length; watcherOffset += 4096) {
            child.stdin.write(watcherScript.substring(watcherOffset, watcherOffset + 4096));
        }
        // Do not call stdin.end(): the bootstrap does not need EOF and starts as soon
        // as the announced number of characters has arrived.
    } catch (writeError) {
        child._printerControlStopping = true;
        replyWatcherStart(child, { success: false, error: "Unable to transfer print-job watcher: " + writeError });
        try { child.kill(); } catch (ignoreWriteKill) { }
        if (jobWatcherProcess === child) jobWatcherProcess = null;
        jobWatcherStartedAt = 0;
        clearWatcherSafetyTimers();
