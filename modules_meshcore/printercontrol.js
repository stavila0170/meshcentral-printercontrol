...
var WATCHER_MARKER = "MESH_PRINTERCONTROL_WATCHER_0427";
...
function buildJobWatcherScript() {
    return [
        ...
        "    if ($nativeAvailable) {",
        "      $signal=0",
        "      try { $signal=$notifier.Wait(1000) } catch { $nativeAvailable=$false; continue }",
        "      if ($signal -ne 0) {",
        "        $burstChanges=0",
        ...
        "      } else {",
        "        $current=Read-PrintJobSnapshot",
        "        if ($null -ne $current) { [void](Publish-PrintJobChanges $current) }",
        "      }",
        "    } else {",
        "      $current=Read-PrintJobSnapshot",
        "      if ($null -ne $current) { [void](Publish-PrintJobChanges $current) }",
