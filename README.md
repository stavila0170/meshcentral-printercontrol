# Mesh Printer Control 0.4.41

Mesh Printer Control adds a **Printers** tab to Windows devices in MeshCentral. Version 0.4.41 runs from the existing LocalSystem Mesh Agent service: it uses the existing LocalSystem **Mesh Agent** service, installs no additional service, and starts its long-running watcher from a temporary PowerShell script that is deleted automatically when monitoring stops. Windows PowerShell may use its normal temporary compiler workspace while loading the native Winspool interop type.
...
Version 0.4.41 displays two distinct sections:
...
For the Docker layout used during development, run these commands from the `MeshPrinterControl-0.4.41` directory. Removing the previous directory first is important because `docker cp` does not delete obsolete assets from older versions:
...
Version 0.4.41 does not recreate this directory.
...
## Active physical printers and hybrid all-printers queue (0.4.41)
