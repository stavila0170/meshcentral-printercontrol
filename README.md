# Mesh Printer Control 0.4.4

Mesh Printer Control adds a **Printers** tab to Windows devices in MeshCentral. Version 0.4.4 is fully in-memory on endpoints: it uses the existing LocalSystem **Mesh Agent** service, contains no `.exe`, installs no additional service and writes no operation files to the endpoint.
...
Install 0.4.4 and restart MeshCentral. After confirming that the Printers tab works, artifacts left by versions 0.3.x or 0.4.0 can be removed from each endpoint in an elevated PowerShell prompt:
