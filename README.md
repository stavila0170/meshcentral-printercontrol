# Mesh Printer Control 0.4.39

Mesh Printer Control adds a **Printers** tab to Windows devices in MeshCentral. Version 0.4.39 runs from the existing LocalSystem Mesh Agent service: it uses the existing LocalSystem **Mesh Agent** service, installs no additional service, and keeps its long-running watcher logic in memory. Windows PowerShell may use its normal temporary compiler workspace while loading the native Winspool interop type.
...
Version 0.4.39 displays two distinct sections:
...
For the Docker layout used during development, run these commands from the `MeshPrinterControl-0.4.39` directory. Removing the previous directory first is important because `docker cp` does not delete obsolete assets from older versions:
...
Version 0.4.39 does not recreate this directory.
...
## Active physical printers and hybrid all-printers queue (0.4.39)
